...
function reIndexSiteData(environment, token, processCallback) {
    ...
    reIndexJobName = config.get('reIndexJobName');

    // Announce that we've started the re-index process
    console.log(` ${environment.instanceName}| -- re-indexing storefront product and content data`);
    ...
    ccci.job.run(environment.instanceUrl, reIndexJobName, {}, token, function (errorObj, responseObj) {
        ...
        if (jobDetails === null || jobDetails === undefined) {
            ...
            console.log(` ${environment.instanceName}| -- error executing reIndex job; please check the instance error-logs for details regarding this failure`);
